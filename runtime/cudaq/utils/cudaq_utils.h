...
void invoke_no_throw(F &&f) {
  try {
    ...
    CUDAQ_DBG("Exception ignored in invoke_no_throw: {}", e.what());
  }
}

/// @brief Invoke `g` after `f`, returning what `f` returns (if any).
///
/// If `f` throws an exception, `g` is called before the exception is rethrown.
/// More specifically:
///  - if `f` does not throw an exception, `auto val = try_finally(f, g);` is
///  equivalent to `auto val = f(); g();`
///  - if `f` throws an exception, it will be rethrown after `g` is called. Any
