...
#include "PythonBackendProvider.h"
...
void PythonBackendProvider::initializeBackendProvider() {
...
void PythonBackendProvider::setTarget(
...
RuntimeTarget PythonBackendProvider::getTarget() {
  auto &provider = RuntimeBackendProvider::getSingleton();
  return provider.getTarget();
}

RuntimeTarget PythonBackendProvider::getTarget(const std::string &name) {
  auto &provider = RuntimeBackendProvider::getSingleton();
  return provider.getTarget(name);
}

std::vector<RuntimeTarget> PythonBackendProvider::getTargets() {
  auto &provider = RuntimeBackendProvider::getSingleton();
  return provider.getTargets();
}

bool PythonBackendProvider::hasTarget(const std::string &name) {
  auto &provider = RuntimeBackendProvider::getSingleton();
  return provider.hasTarget(name);
}

void PythonBackendProvider::resetTarget() {
  auto &provider = RuntimeBackendProvider::getSingleton();
  provider.resetTarget();
}

std::string PythonBackendProvider::getTransportLayer() {
  if (cudaq::__internal__::canModifyTarget()) {
